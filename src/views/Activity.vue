  ...
  <div class="container pt-3.75 pb-7.5">
    <nav aria-label="breadcrumb mb-2.5">
      <ol class="breadcrumb">
        ...
        <li v-if="!isSearch" class="breadcrumb-item active">節慶活動</li>
        <template v-else>
          <li class="breadcrumb-item">
            <router-link to="/activity" @click="reload">節慶活動</router-link>
          </li>
    ...
    <form class="mb-3.75">
      <div class="row">
        ...
        <div class="col-lg-2">
          <input type="text" class="form-control text-primary" placeholder="選擇日期"
            ref="date"/>
        </div>
        <div class="col-lg-3">
...
export default {
  ...
  methods: {
    getActivityData(category, keyword = '') {
      ...
      if (!category) {
        api = `${process.env.VUE_APP_BASEAPI}/Activity?%24filter=(contains(Class1%2C%20'節慶活動')%20or%20contains(Class1%2C%20'自行車活動')%20or%20contains(Class1%2C%20'遊憩活動')%20or%20contains(Class1%2C%20'產業文化活動')%20or%20contains(Class1%2C%20'年度活動')%20or%20contains(Class1%2C%20'四季活動'))%20and%20contains(ActivityName%2C%20'${keyword}')&%24format=JSON`;
    ...
    getActivityByCity(city, category, keyword = '') {
      ...
      if (!category) {
        api = `${process.env.VUE_APP_BASEAPI}/Activity/${city}?%24filter=(contains(Class1%2C%20'節慶活動')%20or%20contains(Class1%2C%20'自行車活動')%20or%20contains(Class1%2C%20'遊憩活動')%20or%20contains(Class1%2C%20'產業文化活動')%20or%20contains(Class1%2C%20'年度活動')%20or%20contains(Class1%2C%20'四季活動'))%20and%20contains(ActivityName%2C%20'${keyword}')&%24format=JSON`;
        this.tempCategory = '自訂搜尋結果';
      }
      this.axios
        .get(api)
